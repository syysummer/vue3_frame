...
export const flatMultiLevelRoutes = (routes: RouteRecordRaw[]) => {
  ...
  routesMirror.forEach((route: RouteRecordRaw) => {
...
const isMultipleRoute = (route: RouteRecordRaw) => {
  ...
  if (children?.length) {
    ...
    return children.some((child: RouteRecordRaw) => child.children?.length)
  }
  return false
}
...
const promoteRouteLevel = (route: RouteRecordRaw) => {
  ...
  route.children = route.children?.map(
    (item: any) => omit(item, "children") as RouteRecordRaw
  )
}
...
const addToChildren = (
  ...
  children.forEach((child: RouteRecordRaw) => {
    const route = routes.find((item: any) => item.name === child.name)
    if (route) {
